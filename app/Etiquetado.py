...
class ProcesarTextov1():
	...
	def ExtraerData(self, sentence):
		ents  = []
		keys1 = []
		keys2 =[]
		print "Sentencia > ",sentence
		ett = self.EtiquetarTT(sentence)
		print ett
	...
	def Desambiguacion(self, listData, listSentence):
	...
	def main(self, text):
		...
		for registro in resultados:
			...
			#if cont == 0: continue
			#if cont == 3: break
			sentences = self.tokenizarSentences(registro)
			for sentence in sentences:
				data['TotalSentences'] = len(sentences)
				extract = self.ExtraerData(sentence)
				listext = []
				for ex in extract:
					...
					for (label, frequency) in nltk.FreqDist(ex).items():
						...
						estruDat["DBpedRList"] = self.extraerListRecursoDBpedia(label)
						listext.append(estruDat)
				#print sentence
				for l in self.Desambiguacion(listext, self.ExtraerNombres(sentence)):
